...
public class RuleConfigSource {
    public RulerConfig load(String filePath){
        ...
        IRuleConfigFactory ruleConfigFactory =  null;
        if("json".equals(fileSuffix)){
            ruleConfigFactory = new JsonRuleConfigFactory();
        }else if ("xml".equals(fileSuffix)){
            ruleConfigFactory = new XmlRuleConfigFactory();
        }else if (!"yaml".equals(fileSuffix)){
            ruleConfigFactory = new YamlRulerConfigFactory();
        }else {
            // TODO: 2023/5/19  自行处理
